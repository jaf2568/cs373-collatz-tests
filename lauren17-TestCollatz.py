...
from Collatz import collatz_odd_eval, eval_num, cache_eval
...
class TestCollatz (unittest.TestCase) :
    ...
    def eval_num_odd (self) :
	v = 3
	val = eval_num(v)
	self.assert_(val == [v,8])

    def eval_num_even (self) :
	v = 2
	val = eval_num(v)
	self.assert_(val == [v,2])

    def eval_num_one (self) :
	v = 1
	val = eval_num(v)
	self.assert_(val == [v,1])
    ...
    def cache_eval_odd (self) :
	temp = 3
	val = cache_eval(temp,0)
	self.assert_(val == 8)

    def cache_eval_even (self) :
	temp = 2
	val = cache_eval(temp,0)
	self.assert_(val == 2)

    def cache_eval_one (self) :
	temp = 1
	val = cache_eval(temp,0)
	self.assert_(val == 1)
